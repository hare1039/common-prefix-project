...
(defdata replica-memory        value-type)
(defdata replica-write-history (listof  value-type))
...
(definec trans-replica-read-req (r :replica dst :address) :trans-return-type ;;TODO
         ...
         :skip-body-contractsp t
         :skip-function-contractp t
         (let* ((val (car (mget :memory r)))
                (response-packet `(READ-RESP ,val)))
           (list r (list (packet
                          (mget :addr r) dst response-packet)))))

(definec generate-packets (target-list :loaddr src :address op :operation) :network
         :skip-body-contractsp t
         :skip-function-contractp t
         (if (endp target-list)
             ()
             (append
              (list (packet src
                            (car target-list)
                            op))
              (generate-packets (cdr target-list) src op))))
...
(definec trans-replica-write-req (r :replica dst :address write-value :value-type) :trans-return-type ;;TODO
         ...
         (let* ((history      (mget :replica-write-history r))
                ...
                (new-memory   write-value)
                (new-replica (mset :replica-memory new-memory
                                   (mset :replica-write-history new-history r)))

                (write-response `(WRITE-RESP))
                (write-response-packet (packet self-address dst write-response))
                (replication-request `(REPL-REQ , (len new-history) , write-value))
                (replication-request-packets (generate-packets self-address dst (mget :backups r))))

           (if (primaryp r)
               (list new-replica
                     (append replication-request-packets write-response-packet))
               (list r '()))))

(definec trans-replica-repl-req (r :replica dst :address d :replica-data) :trans-return-type
         :skip-tests t
         :skip-body-contractsp t
         :skip-function-contractp t
